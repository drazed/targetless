dofile('Roid.lua')
targetls.RoidList = {}
targetls.RoidList.sector = 0
targetls.RoidList.roidcount = 0

function targetls.RoidList:add(id, note, ore)
    local scanned = targetls.Roid:new()
...
function targetls.RoidList:refresh()
    -- regenerate iups 
    for i,v in ipairs(self) do 
        v:destroy()
    end
    for i,v in ipairs(self) do 
        if(i>targetls.var.listmax) then return end
        ...
        if(objectid and v["id"] == ""..objectid) then 
            ...
            numlabel.font = targetls.var.font 
        ...
        v["label"] = iupbox
        iup.Append(targetls.var.iuproids, v["label"])
        iup.Map(v["label"])
    end
end

function targetls.RoidList:updatesector(sectorid)
    if(sectorid) then
        ...
        -- updates
        if(targetls.var.listpage == "roid") then
            self:refresh()
        end
        self.roidcount = #self
    end
...
function targetls.RoidList:clear()
    while(self[1]) do
        self[1]:destroy()
        table.remove(self,1)
    end
...
function targetls.RoidList:savesector(sectorID)
    local roids = ""
    for i,v in ipairs(self) do 
        roids = roids.."<"..v['id']..">"
...
function targetls.RoidList:loadsector(sectorID)
    -- merge the list from older versions of targetLS
    self:mergeold(sectorID)
...
-- Remove this a few versions from now ;)
function targetls.RoidList:mergeold(sectorID)
    local string = gkini.ReadString("roidls", ""..sectorID, "")
    if string == "" then return end -- nothing to merge
    local data = {}
    string.gsub(string,"{(.-)}", function(a) table.insert(data, a) end)
    for i,v in ipairs(data) do 
        local roid = {}
        string.gsub(v,"<(.-)>", function(a) table.insert(roid, a) end)
        self:add(roid[1], roid[2], roid[3])
    end
    -- there was a sector to merge, main area should be clear to overwrite
    self:savesector(sectorID)
    gkini.WriteString("roidls",""..sectorID,"")
end
